...
import { Search, Phone, ChevronDown, Home, Menu, X, ArrowRight } from 'lucide-react';
...
const Navbar = () => {
    ...
    const specialitiesData = [
        ...
        { id: "spec-006", title: "Cardiothoracic & Vascular Surgery" },
        { id: "spec-007", title: "Counselling Center" },
        { id: "spec-008", title: "Critical Care Unit" },
        { id: "spec-009", title: "Dental & Maxillofacial Surgery" },
        { id: "spec-010", title: "Dermatology & Venereology" },
        { id: "spec-011", title: "Dietetics & Nutrition" },
        { id: "spec-012", title: "Endocrinology & Diabetology" },
        { id: "spec-013", title: "ENT & Head Neck Surgery" },
        { id: "spec-014", title: "Gastroenterology & Hepatology" },
        { id: "spec-015", title: "General & Laparoscopic Surgery" },
        ...
        { id: "spec-022", title: "Obstetrics And Gynaecology" },
        { id: "spec-023", title: "Ophthalmology" },
        { id: "spec-024", title: "Orthopaedics & Joint Replacement" },
        { id: "spec-025", title: "Paediatrics" },
        { id: "spec-026", title: "Radiology & Imaging" },
        { id: "spec-027", title: "Urology" }
    ];

    // Mega Menu Column logic 
    const columns = [];
    ...
    return (
        <header className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
            ...
            {/* TOP BAR */}
            <div className={`bg-[#002B49] text-white px-4 md:px-10 flex justify-between items-center transition-all duration-500 overflow-hidden ${scrolled ? 'h-0 opacity-0' : 'h-10 opacity-100'}`}>
                <div className="flex items-center gap-4">
                    <Link to="/" className="bg-[#4FC3F7] px-3 py-1 rounded flex items-center gap-2 font-bold text-[#002B49] text-xs transition-colors hover:bg-white">
                        <Home size={14} /> Evercarebd
                    </Link>
                </div>
                <div className="flex items-center gap-2 font-black text-sm">
                    <Phone size={16} className="text-[#4FC3F7]" /> 10663
                </div>
            </div>

            {/* MAIN NAVBAR */}
            <div className={`px-4 md:px-10 flex justify-between items-center h-[75px] transition-all duration-500 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-lg' : 'bg-white shadow-sm'}`}>
                <Link to="/" className="flex flex-col">
                    <h1 className="text-3xl font-black text-[#002B49] leading-none">evercare<span className="text-[#4FC3F7]">.</span></h1>
                    <p className="text-[10px] font-bold text-[#002B49] tracking-widest uppercase">Hospital</p>
                </Link>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button 
                        ...
                        className="lg:hidden p-2 bg-gray-100 rounded-lg text-[#002B49]"
                    >
                        <Menu size={24} />
                    </button>
                </div>
                ...
                {isMobileMenuOpen && (
                    <div className="fixed inset-0 z-[999] lg:hidden h-screen w-screen">
                        ...
                        <motion.div 
                            ...
                            className="absolute right-0 top-0 h-full w-[85%] max-w-[350px] bg-white flex flex-col shadow-2xl"
                        >
                            {/* Header */}
                            ...
                            {/* Nav Links Area  */}
                            <div className="flex-1 overflow-y-auto pt-4 bg-white">
                                {navLinks.map((item) => (
                                        ...
                                        {item.hasDropdown ? (
                                            <>
                                                <button 
                                                    ...
                                                    className="w-full flex items-center justify-between px-8 py-5 text-[15px] font-bold text-[#002B49]"
                                                >
                                                    {item.name} 
                                                    <ChevronDown size={18} className={`transition-transform duration-300 ${isMobileSpecOpen ? 'rotate-180' : ''}`} />
                                                </button>
                                                            ...
                                                            <div className="py-2">
                                                                {specialitiesData.map((spec) => (
                                                                    <Link 
                                                                        ...
                                                                        to={`/spec/${spec.id}`} 
                                                                        onClick={() => setIsMobileMenuOpen(false)} 
                                                                        className="block px-12 py-3 text-sm text-gray-600 hover:text-[#002B49] transition-colors"
                                                                    >
                                                                        {spec.title}
                                                                    </Link>
                                                                ))}
                                            ...
                                            <Link 
                                                to={item.path} 
                                                onClick={() => setIsMobileMenuOpen(false)} 
                                                className="flex items-center justify-between px-8 py-5 text-[15px] font-bold text-[#002B49] hover:bg-gray-50 transition-colors"
                                            >
                                                {item.name} <ArrowRight size={16} className="text-[#4FC3F7] opacity-60" />
                                            </Link>
                                        )}
                                    </div>
                                ))}
                            ...
                            <div className="p-6 bg-gray-50 border-t shrink-0">
                                <Link 
                                    to="/contact" 
                                    onClick={() => setIsMobileMenuOpen(false)} 
                                    className="w-full bg-[#002B49] text-white py-4 rounded-xl font-bold flex justify-center shadow-lg active:scale-95 transition-all uppercase text-sm tracking-widest"
                                >
                                    Query
                                </Link>
                            </div>
